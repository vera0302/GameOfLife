...
public class MainPanel extends JPanel {
    ...
    {
    	// Second change: I removed some codes from here, which are time-consuming.
    	// Since the defined parameter is already an int type, there's not need to convert to int repeatly.
    	// We should just simply return the value, that's what we expect.
    	// In this case, I keep this method exist so as to avoid other bugs of other functions calling issue.
    	return x;
    }

    // Adding this method which can be used in the pinning tests for the original function. Testify if it changes the functionality.
    ...
    public void backup()
    {
    	// Third change: The original new object was created at each time from the original method.
    	// Here I changed the method and create the new backupCell with the active cell's getAlive() function.
	    for (int j = 0; j < _size; j++)
	    {
	        for (int k = 0; k < _size; k++)
	        {
    ...
    // Adding this method which is in the pinning tests for the functionality of the backup method.
    ...
    public void runContinuous()
    {
	    ...
	    while (_running)
	    {
		    // Forth change: I deleted the original loop, which was supposed to change the value of _r and then reseting it to the original value after the loop.
		    // This function significantly runs a lot CPU time as shown in VisualVM, which is naturally caused.
	    	// However, deleting the unnecessary loop will definitely save much time to take more runnings of the game.
	    	// I don't give pinning tests for this modification as there are neither input nor output values in this function.
	...
	// Final change: I make the backupCell array here with new cells filled-in.
	// In this way, we can save the running time from making new cells for each one backup.
	...
	for (int j = 0; j < size; j++)
	{
	    ...
	    {
	    	// change here for the same reason, too.
